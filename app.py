...
<style>
    ...
    .grounding-notice {
        background: #e8f4e8; border-left: 4px solid #2e7d32;
...
with st.sidebar:
    ...
    with st.expander("🔑 API Key Configuration", expanded=False):
        st.markdown(
            "Each feature uses its own API key to isolate usage. "
            "If a dedicated key is blank, the **Master Key** is used as fallback."
        )
        master_key  = st.text_input("Master / Fallback Key",   type="password", key="key_master",  placeholder="sk-ant-...")
        audit_key   = st.text_input("Standard Reviewer Key",   type="password", key="key_audit",   placeholder="sk-ant-... (optional)")
        nqf_key     = st.text_input("NQF Alignment Key",       type="password", key="key_nqf",     placeholder="sk-ant-... (optional)")
        ssr_key     = st.text_input("SSR Writer Key",          type="password", key="key_ssr",     placeholder="sk-ant-... (optional)")
        extract_key = st.text_input("Evidence Extractor Key",  type="password", key="key_extract", placeholder="sk-ant-... (optional)")

        if st.button("💾 Save Keys to Session"):
            import os
            if master_key:  os.environ["ANTHROPIC_API_KEY"]         = master_key
            if audit_key:   os.environ["ANTHROPIC_API_KEY_AUDIT"]   = audit_key
            if nqf_key:     os.environ["ANTHROPIC_API_KEY_NQF"]     = nqf_key
            if ssr_key:     os.environ["ANTHROPIC_API_KEY_SSR"]     = ssr_key
            if extract_key: os.environ["ANTHROPIC_API_KEY_EXTRACT"] = extract_key
    ...
    if "processed_chunks" not in st.session_state:
        st.session_state.processed_chunks = {}   # {filename: [chunk, chunk, ...]}
        st.session_state.all_chunks = []          # flat list of all chunks across all docs
        st.session_state.full_text = ""           # kept for legacy NQF paste fallback
    ...
    if uploaded_files and st.button("📖 Process Documents"):
        with st.spinner("Reading and indexing documents..."):
            ...
            st.success(f"✅ Indexed {len(processed)} document(s) — {total_pages} pages total.")
...
st.markdown(
    ...
    'Anything not evidenced will be explicitly flagged as <em>NOT EVIDENCED IN DOCUMENTS</em>.</div>',
    unsafe_allow_html=True
)
...
with tab1:
    ...
    if not st.session_state.processed_chunks:
        ...
        found_docs = [
            ...
            if any(doc.lower().split()[0] in f.lower() for f in uploaded_names)
        ]
...
with tab2:
    ...
    st.markdown(
        "The reviewer scans **every page** of your documents for evidence relevant to the selected "
        "standard before generating its assessment. Only evidence found in your files is used."
    )
    ...
    if st.button("▶️ Run Compliance Audit"):
        if not st.session_state.all_chunks:
            ...
            if not client:
                st.error("No API key found. Add your key in the sidebar.")
            else:
                with st.spinner(
                    f"Phase 1/2: Extracting relevant passages for '{selected_standard}'…"
                ):
    ...
    if st.session_state.audit_result:
        ...
        if "error" in analysis:
            ...
            s1, s2 = st.columns(2)
            with s1:
                st.success("✅ Strengths (with document citations)")
            ...
            with s2:
                ...
                gaps = analysis.get("areas_for_improvement", [])
                for gap in gaps:
                    st.write(f"- {gap}")
            ...
            st.download_button(
                ...
                f"audit_{selected_standard.replace(' ', '_')}.json",
                "application/json"
            )
...
with tab3:
    ...
    st.markdown(
        "Paste your PLOs directly, or leave blank to use PLO content extracted from your uploaded documents."
    )
        ...
        if not plo_source:
            ...
            if not client:
                ...
                with st.spinner(f"Analysing alignment from {source_label}…"):
                    ...
                    st.markdown(result)
                    st.download_button("📥 Download NQF Report", result, "nqf_alignment.md")
...
with tab4:
    ...
    user_query = st.chat_input("Ask the SSR Assistant (e.g. 'Write the narrative for Standard 4 — Students')…")
    ...
    if user_query:
        ...
        if not st.session_state.all_chunks:
            ...
            if not client:
                reply = "❌ No API key configured. Please add your SSR Writer key in the sidebar."
            else:
                with st.chat_message("assistant"):
                    with st.spinner("Searching documents and drafting response…"):
                        ...
                        st.download_button(
                            ...
                            reply,
                            f"ssr_response.txt",
                            key=f"dl_{len(st.session_state.ssr_chat_history)}"
                        )
