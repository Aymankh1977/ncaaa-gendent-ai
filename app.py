...
import json
from config import RUBRIC_CRITERIA, NCAAA_STANDARDS_GROUPS, NQF_DOMAINS, REQUIRED_DOCUMENTS, ALL_CRITERIA_KEYS
...
<style>
    ...
    .score-4 { background:#1b5e20; color:white; padding:3px 8px; border-radius:4px; font-weight:bold; }
    .score-3 { background:#2e7d32; color:white; padding:3px 8px; border-radius:4px; font-weight:bold; }
    .score-2 { background:#f57f17; color:white; padding:3px 8px; border-radius:4px; font-weight:bold; }
    .score-1 { background:#b71c1c; color:white; padding:3px 8px; border-radius:4px; font-weight:bold; }
</style>
...
with tab2:
    ...
    if not st.session_state.processed_chunks:
        ...
        if not audit_docs:
            ...
            # Two-level selector: Standard group → individual criterion
    ...
    if st.session_state.audit_result:
        ...
        if "error" in analysis:
            ...
            # PDF download
            selected_crit_for_dl = st.session_state.get("audit_last_criterion", selected_criterion if 'selected_criterion' in dir() else "Criterion")
...
with tab3:
    ...
    if not st.session_state.processed_chunks:
        ...
        st.markdown(
            ...
            'Course Specification — not the official NQF document itself.</div>',
            unsafe_allow_html=True
        )
...
with tab4:
    ...
    if not st.session_state.processed_chunks:
        ...
        if ssr_docs:
            with st.expander(f"📋 {len(ssr_docs)} document(s) selected"):
    ...
    # Persistent download button for latest reply
    if st.session_state.get("ssr_last_reply"):
        ...
        st.download_button(
            "📥 Download Last Response (PDF)", data=pdf_bytes,
            file_name="ssr_section.pdf", mime="application/pdf",
            key="ssr_dl_persistent"
        )

    user_query = st.chat_input("Ask the SSR Assistant…")
